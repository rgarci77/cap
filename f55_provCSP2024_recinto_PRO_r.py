...
# Ruta de trabajo (Rasters_CSP)
arcpy.env.workspace = dirPrin

# CONSTRUCCIÓN DE RUTAS Y CONTROL DE EXISTENCIA
carpeta_prov = os.path.join(dirPrin, f"prov{codProv}")
gdb_trabajo  = os.path.join(carpeta_prov, f"prov{codProv}.gdb")

# 1. CONTROL DE ERRORES: Comprobamos si la carpeta de la provincia existe
if not os.path.exists(carpeta_prov):
    arcpy.AddError(f"ERROR CRÍTICO: No existe la carpeta de la provincia seleccionada: {carpeta_prov}")
    raise SystemExit

# 2. CONTROL DE ERRORES: Comprobamos si la File Geodatabase (.gdb) existe
if not arcpy.Exists(gdb_trabajo):
    arcpy.AddError(f"ERROR CRÍTICO: No existe la Geodatabase de trabajo: {gdb_trabajo}")
    raise SystemExit
...
with open(ruta_log, "a", encoding="utf-8") as fictime:
    fictime.write('*******************************\n')
    mensaje_inicio = f"Inicio la provincia: {codProv} el {time.ctime()}"
    
    arcpy.AddMessage(mensaje_inicio)
    fictime.write(f"{mensaje_inicio}\n")

    # *********************************
    # Configuro entornos geográficos basados en la capa límite
    prov_ras = os.path.join(gdb_trabajo, f"GLIMITE_{codProv}")

    # 3. CONTROL DE ERRORES: Verificación de la capa límite
    if arcpy.Exists(prov_ras):
        dsc = arcpy.Describe(prov_ras)
        coord_sys = dsc.spatialReference
        # Se asegura de trabajar en la misma proyección que los grid raster
        arcpy.env.outputCoordinateSystem = coord_sys
    else:
        arcpy.AddError(f"ERROR CRÍTICO: No se ha encontrado la capa base obligatoria dentro de la GDB: {prov_ras}")
        raise SystemExit


#%% proceso 2

    # Definimos la ruta de la Feature Class destino dentro de la GDB de trabajo
    # Es altamente recomendable que 'PROYE' sea un Feature Dataset existente o se use la ruta directa de la GDB
    subcarpeta_proye = os.path.join(gdb_trabajo, "PROYE")
    vshp_recp = os.path.join(subcarpeta_proye, f"precfe_{codProv}")
    arcpy.AddMessage(vshp_recp)

    #recinto sigpac
    if int(mun_ini)  == 1:
        arcpy.AddMessage("muni_ini = 1 -> Exportando y uniendo tablas...")
        
        # Validamos que exista el dataset de destino o la GDB base
        if not arcpy.Exists(subcarpeta_proye):
            # Si 'PROYE' no existe en la GDB, puedes crearlo como Feature Dataset o guardar directamente en la GDB
            # Por seguridad, si falla creamos la ruta directa en la GDB removiendo 'PROYE' si no es un dataset real
            vshp_recp = os.path.join(gdb_trabajo, f"precfe_{codProv}")
            subcarpeta_proye = gdb_trabajo
            
        # Conversión de Feature Class protegiendo proyecciones
        arcpy.conversion.FeatureClassToFeatureClass(shp_rec, subcarpeta_proye, f"precfe_{codProv}")
        arcpy.management.JoinField(vshp_recp, "DN_OID", bd_rec, "DN_OID")

    else:
        arcpy.AddMessage("muni_ini distinto de 1 -> Reutilizando capa existente.")
        # SOLUCIÓN AL ERROR 000192: Borrado seguro del índice mediante control de excepciones
        try:
            if arcpy.Exists(vshp_recp):
                arcpy.management.RemoveIndex(vshp_recp, ["iID_UNICO"])
                arcpy.AddMessage("Índice 'iID_UNICO' eliminado correctamente.")
            else:
                arcpy.AddError(f"ERROR CRÍTICO: No se encuentra la capa necesaria para continuar: {vshp_recp}")
                raise SystemExit

        except arcpy.ExecuteError:
            # Si el índice no existe, ArcGIS Pro arroja un error pero dejamos que el script continúe sin caerse
            arcpy.AddWarning("El índice 'iID_UNICO' no existía en la capa. Se omite el paso.")
    ...
    # Configuración de entornos de Geoprocesamiento espaciales
    ...
    # ***************************************************
    # COMPROBACIÓN DE RÁSTERES OBLIGATORIOS
        
    # 1. Definimos una lista con los nombres de todos los rásteres requeridos en MAYÚSCULAS
    rasters_requeridos = [
        f"GMDT_{codProv}",
        f"GPTE_{codProv}",
        f"GCSP_{codProv}",
    ...
    # 2. Buscamos cuáles de ellos no existen físicamente en la GDB
    rasters_ausentes = []
    for nombre_raster in rasters_requeridos:
        ruta_completa = os.path.join(gdb_trabajo, nombre_raster)
        if not arcpy.Exists(ruta_completa):
            rasters_ausentes.append(nombre_raster)
    
    # 3. Si la lista de ausentes tiene elementos, avisamos al usuario y abortamos
    if rasters_ausentes:
        arcpy.AddError("ERROR CRÍTICO: Faltan rásteres obligatorios en la Geodatabase.")
        fictime.write("ERROR: Faltan rásteres obligatorios en la Geodatabase:\n")
        
        for ausente in rasters_ausentes:
            arcpy.AddError(f" -> No se encuentra el ráster: {ausente}")
            fictime.write(f" -> Ausente: {ausente}\n")
            
        raise SystemExit  # Detiene el script de forma limpia
        
    # 4. Si todos existen, se instancian con total seguridad de que no darán error
    arcpy.AddMessage("Todos los rásteres requeridos han sido verificados con éxito.")

    # Instanciación explícita de Rásteres apuntando a la GDB de trabajo
    vgalt = Raster(os.path.join(gdb_trabajo, f"GMDT_{codProv}")) 
    vgpte = Raster(os.path.join(gdb_trabajo, f"GPTE_{codProv}")) 
    vgcsp = Raster(os.path.join(gdb_trabajo, f"GCSP_{codProv}"))
    vgfsue = Raster(os.path.join(gdb_trabajo, f"GFSUE_{codProv}"))
    vgfpte = Raster(os.path.join(gdb_trabajo, f"GFPTE_{codProv}"))
    vgfveg = Raster(os.path.join(gdb_trabajo, f"GFVEG_{codProv}"))
    vgfveges = Raster(os.path.join(gdb_trabajo, f"GFVEGESP_{codProv}"))
    vgfinc = Raster(os.path.join(gdb_trabajo, f"GFINC_{codProv}"))
...
#%% Proceso 3
    ...
    if int(mun_ini)  == 1:
        ...
        arcpy.AddMessage("Inicializando campos estructurales de salida...")
        ...
        arcpy.CalculateField_management(vshp_recp,"ID_UNICO","!OBJECTID!", "PYTHON3")
        arcpy.CalculateField_management(vshp_recp,"ALT_NEW","-1", "PYTHON3") #20221020
        arcpy.CalculateField_management(vshp_recp,"PTE","-1", "PYTHON3") #20211116
        arcpy.CalculateField_management(vshp_recp,"CSP","-1", "PYTHON3")
        arcpy.CalculateField_management(vshp_recp,"F_SUE","-1", "PYTHON3")
        arcpy.CalculateField_management(vshp_recp,"F_PTE","-1", "PYTHON3")
        arcpy.CalculateField_management(vshp_recp,"F_VEG","-1", "PYTHON3")
        arcpy.CalculateField_management(vshp_recp,"POR_DEH","0", "PYTHON3") #20211125
        arcpy.CalculateField_management(vshp_recp,"POR_FESPDE","0", "PYTHON3") #20211111
        arcpy.CalculateField_management(vshp_recp,"POR_ESP","0", "PYTHON3") #20211125
        arcpy.CalculateField_management(vshp_recp,"POR_FESPES","0", "PYTHON3") #20211111
        arcpy.CalculateField_management(vshp_recp,"POR_FESP","0", "PYTHON3") #20211111
        arcpy.CalculateField_management(vshp_recp,"F_VEGES","-1", "PYTHON3")
        arcpy.CalculateField_management(vshp_recp,"F_INC","-1", "PYTHON3")
        arcpy.CalculateField_management(vshp_recp,"CSP_RES","-1", "PYTHON3")
        arcpy.CalculateField_management(vshp_recp,"SOLAPES","0", "PYTHON3")
        arcpy.CalculateField_management(vshp_recp,"SOLAPES2","0", "PYTHON3")

    # Reparo geometría (Controlando variaciones de mayúsculas en el booleano)
    if repgeo.lower() == "true":
        arcpy.AddMessage("Reparando geometría de la capa de recintos...")
        tabla_errores_geo = os.path.join(gdb_trabajo, f"ErrGeoSigp_{codProv}")
        arcpy.management.CheckGeometry(vshp_recp, tabla_errores_geo)
        arcpy.RepairGeometry_management(vshp_recp)

    # Introducción de coordenadas del geocentro (ETRS89)
    ...
    # Generación de la Feature Layer de trabajo e indexación
    ...
    # Saco el listado único de municipios a procesar en la GDB de trabajo
    tabla_frecuencia = os.path.join(gdb_trabajo, "recfe_txt2")
    arcpy.analysis.Frequency("lyshprec", tabla_frecuencia, ["MUNICIPIO"])

    #arcpy.Frequency_analysis("lyshprec", "recfe_txt2",["MUNICIPIO"])
    nunmun = int(arcpy.GetCount_management("recfe_txt2").getOutput(0))

    arcpy.AddMessage(f"Número de municipios únicos detectados: {nunmun}")
    fictime.write(f"Número de municipios unicos detectados: {nunmun}\n")
